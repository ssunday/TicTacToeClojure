...
(defn get-player-spot-to-be-marked [board]
  ...
  (loop [spot (validation/convert-string-to-number (read-line))
         board board]
    (if (validation/check-if-spot-is-invalid board spot)
      (do (cond (validation/check-if-spot-is-invalid-input board spot) (println (translate loc :error-messages/spot-is-invalid-input))
                (validation/check-if-spot-is-not-on-board board spot) (println (translate loc :error-messages/spot-is-not-on-board))
                (validation/check-if-spot-is-already-marked board spot) (println (translate loc :error-messages/spot-not-open)))
          (recur (validation/convert-string-to-number (read-line)) board))
      spot)))
...
(defn get-player-one-marker []
  ...
  (loop [player-one-marker (read-line)]
    (if (validation/check-if-marker-is-invalid player-one-marker)
        (do
...
(defn get-player-two-marker [player-one-marker]
  ...
  (loop [player-two-marker (read-line)]
    (if (or  (= player-two-marker player-one-marker)
             (validation/check-if-marker-is-invalid player-two-marker))
        (do
...
(defn get-first-player [player-one-marker player-two-marker]
  ...
  (loop [first-player-marker (clojure.string/upper-case (read-line))]
    (if (validation/check-if-first-player-marker-is-invalid first-player-marker player-one-marker player-two-marker)
        (do
  ...
  (loop [board-type (read-line)]
    (if (and (not= board-type "3")
        ...
        (do
          ...
          (recur (read-line)))
        (validation/convert-string-to-number board-type))))

(defn yes-or-no-loop []
  (loop [y-or-n (read-line)]
    (if (validation/check-if-yes-or-no-response-is-invalid y-or-n (translate loc :input/yes-option) (translate loc :input/no-option))
        (do
...
(defn display-tally [tally]
  (println (translate loc :output/player-tally) (translate loc :output/tally-header))
...
(defn select-menu-option [menu-options]
  ...
  (loop [option (validation/convert-string-to-number (read-line))]
        (if (validation/check-if-menu-option-is-valid menu-options option)
            option
